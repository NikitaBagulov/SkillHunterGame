	...
	if Engine.is_editor_hint():
		if patrol_locations.size() > 0:
			for i in patrol_locations.size():
				var _patrol = patrol_locations[i] as PatrolLocation
				if not _patrol.transform_changed.is_connected(gather_patrol_locations):
					_patrol.transform_changed.connect(gather_patrol_locations)
				_patrol.update_label(str(i))
				_patrol.modulate = _get_color_by_index(i)
				...
				_patrol.update_line(_next.position)	
